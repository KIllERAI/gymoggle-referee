...
def _update_player(p, exercise, won, drew, record_result=True):
    ...
    try:
        ...
        if exercise in ("squats", "pushups", "situps", "jacks", "plank"):
            col = "best_" + exercise
            payload[col] = max(row.get(col, 0), p.get("reps", 0))
            # cumulative lifetime totals (also count real effort vs bots)
            tcol = "total_" + exercise
            payload[tcol] = (row.get(tcol, 0) or 0) + int(p.get("reps", 0))
        # lifetime matches played (only real matches, via record_result)
        if record_result:
            payload["total_matches"] = (row.get("total_matches", 0) or 0) + 1
        supabase.table("players").upsert(payload).execute()
    except Exception as e:
        print("update_player failed:", e)
...
async def run_match(code):
    ...
    # a bot always LEAVES after the game — so "Play again" finds a fresh opponent
    # (a new bot or a real human) instead of replaying the same bot.
    bot_ws = next((w for w in list(room["players"].keys()) if is_bot(w)), None)
    if bot_ws is not None:
        room["players"].pop(bot_ws, None)
        ident.pop(bot_ws, None)
        room["has_bot"] = False
...
async def start_lobby(code):
    ...
    if not room or room.get("started") or len(room["players"]) != 2:
        return
    room["in_lobby"] = True
    for ws2, p in room["players"].items():
        p["lobby_ready"] = is_bot(ws2)      # a bot is ALWAYS ready the moment it's in the room
    names = {p["id"]: p["name"] for p in room["players"].values()}
    await broadcast(room, {"type": "lobby", "duration": LOBBY_SECS, "names": names})
    room["lobby_task"] = asyncio.create_task(lobby_timer(code))
    # if a bot is in the room, have it drop one trash-talk line after a beat
...
@app.post("/me/stats")
def me_stats(payload: dict):
    """Full personal stats for the logged-in user (for the leaderboard's My Stats)."""
    user = verify_token((payload or {}).get("token", ""))
    if not user:
        return {"ok": False, "error": "unauthorized"}
    try:
        r = supabase.table("players").select("*").eq("id", user["id"]).execute()
        if not r.data:
            return {"ok": True, "stats": None}
        row = r.data[0]
        def g(k): return row.get(k, 0) or 0
        return {"ok": True, "stats": {
            "name": row.get("name", "Player"),
            "wins": g("wins"), "losses": g("losses"), "draws": g("draws"),
            "total_matches": g("total_matches"),
            "best_streak": g("best_streak"), "current_streak": g("current_streak"),
            "coins": g("coins"),
            "best":  {"squats": g("best_squats"), "pushups": g("best_pushups"),
                      "jacks": g("best_jacks"), "plank": g("best_plank")},
            "total": {"squats": g("total_squats"), "pushups": g("total_pushups"),
                      "jacks": g("total_jacks"), "plank": g("total_plank")},
        }}
    except Exception as e:
        print("me_stats error:", e)
        return {"ok": False, "error": "server"}
